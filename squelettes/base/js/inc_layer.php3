...
function afficher_script_layer(){
	...
	if (test_layer()){
		echo '<script type="text/javascript" src="'.($flag_ecrire ? "" : "base/js/").'layer.js">';
		echo "</script>\n";
	}
}
...
function bouton_block_invisible($nom_block) {
	...
	if (test_layer()) {
		...
		$retour = "\n<script type='text/javascript'><!--\n";
		$retour .= "document.write('<a href=\"javascript:$javasc\"><img name=\"triangle".$numero_block["$nom_block"]."\" src=\"base/images/symboleIP-plus$spip_lang_rtl.png\" alt=\"\" title=\"".addslashes(_T('info_deplier'))."\"border=\"0\"></a> ');\n";
		$retour .= "//-->\n";
		$retour .= "</script>\n";

		//$retour .= "<noscript><img name='triangle$num_triangle' src='base/images/symboleIP-moins$spip_lang_rtl.png' alt='' border='0'></noscript>\n";
		return $retour;
	}
}
...
function bouton_block_visible($nom_block){
	...
	if (test_layer()){
		...
		return "<a href=\"javascript:swap_couche('".$numero_block["$nom_block"]."', '$spip_lang_rtl')\"><IMG name='triangle".$numero_block["$nom_block"]."' src='base/images/symboleIP-moins$spip_lang_rtl.png' alt='' title='".addslashes(_T('info_deplier'))."'border='0'></a> ";
	}
}
